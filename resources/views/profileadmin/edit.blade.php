...
<form action="{{ route('profileadmin.update', $admin->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        <label for="teacherName" class="form-label">Nama Admin</label>
        <input type="text" class="form-control" id="teacherName" name="nama" value="{{ old('nama', $admin->nama) }}" required>
    </div>
    <div class="mb-3">
        <label for="teacherPhoto" class="form-label">Foto Admin</label>
        <input type="file" class="form-control" id="teacherPhoto" name="image_url">
        @if($admin->image_url)
            <div class="mt-2">
                <img src="{{ asset('storage/' . $admin->image_url) }}" alt="Foto Kakom" class="img-thumbnail" style="width: 150px;">
            </div>
        @endif
    </div>
    <div class="mb-3">
        <label for="teacherWhatsApp" class="form-label">WhatsApp</label>
        <input type="number" class="form-control" id="teacherWhatsApp" name="no_wa" value="{{ old('no_wa', $admin->no_wa) }}" required>
    </div>
    <div class="mb-3">
